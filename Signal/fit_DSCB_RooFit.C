...
void fit_DSCB_RooFit()
{
    ...
    // ----------------------------------------------------------
    // Find the bin numbers corresponding to the range [700, 4000]
    int bin_start = h->GetXaxis()->FindBin(700.0);
    int bin_end   = h->GetXaxis()->FindBin(4000.0);
    ...
    std::cout << "Signal Yield in [700, 4000]: " << Nsig << std::endl;
