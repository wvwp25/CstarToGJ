...
void CstarToGJAnalysis::Loop(const char *outputFile, double crossSectionPb)
{
    ...
    for (Long64_t jentry=0; jentry<nentries;jentry++) {
        ...
        // Dedicated 2017 high-pT (pT > 200 GeV) Medium photon-ID SFs
        // used in AN2019-267/JHEP12(2023)189. The associated 10% high-pT
        // photon-efficiency uncertainty is a normalization nuisance.
        double photonIDSF = 1.0;
        double absPhoEta = std::abs(pho_eta);
        if (absPhoEta < 0.8) {
            photonIDSF = 1.019;
        } else if (absPhoEta < 1.4442) {
            photonIDSF = 1.015;
        }
        ...
        for (UInt_t i= 0; i < nJet; ++i){
            ...
            double ptAfterJes   = Jet_pt[i]   * jesNom;
            double massAfterJes = Jet_mass[i] * jesNom;
        ...
        h_sig->Fill(m_nom, weight_nominal);

        h_sig_PUUp->Fill(m_nom, weight_PUUp * ctagSF_nom * photonIDSF);
        h_sig_PUDown->Fill(m_nom, weight_PUDown * ctagSF_nom * photonIDSF);

        h_sig_JERUp->Fill(m_JERUp, weight_nominal);
        h_sig_JERDown->Fill(m_JERDown, weight_nominal);

        h_sig_JESUp->Fill(m_JESUp, weight_nominal);
        h_sig_JESDown->Fill(m_JESDown, weight_nominal);

        h_sig_PESUp->Fill(m_PESUp, weight_nominal);
        h_sig_PESDown->Fill(m_PESDown, weight_nominal);

        h_sig_CTagUp->Fill(m_nom, weight_CTagUp * photonIDSF);
        h_sig_CTagDown->Fill(m_nom, weight_CTagDown * photonIDSF);
