...
auto FindGoodPhotons(T &df) {
    return df.Define("goodPhoton","Photon_pt > 240 && abs(Photon_eta) < 1.4442").Filter("Sum(goodPhoton)>=1");
}

template <typename T>
auto FindKinematicCuts(T &df) {
    // Jet Selection
    auto df_kin = df.Define("goodJet","(Jet_pt > 170) && (abs(Jet_eta) < 2.4) && (Jet_deltaR_photon > 1.1) && (Jet_deltaEta_photon < 1.5)")
        .Filter("Sum(goodJet)>=1");
    return df_kin.Define("Trig_Decision","HLT_Photon200==1").Filter("Trig_Decision==1");
}
void ForTrigSF(string inFile, string outFile){
    ...
    df.Redefine("run",  "run")
        ...
        .Redefine("Jet_btagDeepFlavourB", "Jet_btagDeepFlavourB") // For B-tagging
        .Redefine("Jet_btagDeepFlavourC", "Jet_btagDeepFlavourC") // For C-tagging
        .Redefine("Jet_btagDeepFlavourL", "Jet_btagDeepFlavourL") // For Light-tagging
    ...
    df.Snapshot("Events", filename2, {
            ...
            "Jet_phi",
            "Jet_pt",
            "Jet_btagDeepFlavourB",
            "Jet_btagDeepFlavourC",
            "Jet_btagDeepFlavourL",
            "Jet_rawFactor",
            "TrigObj_l1charge",
            "TrigObj_id",
            "TrigObj_l1iso",
