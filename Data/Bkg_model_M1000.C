...
// helper: compute chi2 over selected ranges using histogram errors
static void ComputeChi2InSidebands(const TH1* h, const TF1* f,
                                   ...
                                   double &chi2, int &ndof)
{
  ...
  for (int i = 1; i <= nb; ++i) {
    ...
    // If you use Poisson data and want chi2-like metric, skip bins with 0 error.
    if (err <= 0) continue;
  ...
  // subtract number of free params (approx ndof)
  ndof -= f->GetNpar();
  if (ndof < 1) ndof = 1;
}
...
void Bkg_model_M1000() {
  ...
  if (!fin || fin->IsZombie()) {
    std::cerr << "[ERROR] Cannot open Run2017BCDEF_BG_ana.root\n";
    return;
  }

  TH1F *hM_in = dynamic_cast<TH1F*>(fin->Get("hM"));
  if (!hM_in) {
    std::cerr << "[ERROR] Cannot find histogram 'hM' in input file\n";
    fin->Close();
    return;
  }
  ...
  // Make sure errors exist (in case input forgot Sumw2)
  hM->Sumw2();
  ...
  TFitResultPtr r1 = hM->Fit(bkgFit, "IRS0", "", fit_low1, fit_high1);
  if (int(r1) != 0) std::cerr << "[WARN] Fit in SB1 returned code " << int(r1) << "\n";

  TFitResultPtr r2 = hM->Fit(bkgFit, "IRS0+", "", fit_low2, fit_high2);
  if (int(r2) != 0) std::cerr << "[WARN] Fit in SB2 returned code " << int(r2) << "\n";

  // Compute chi2/ndof over both sidebands consistently
  double chi2 = 0.0;
  int ndof = 0;
  ComputeChi2InSidebands(hM, bkgFit, fit_low1, fit_high1, fit_low2, fit_high2, chi2, ndof);
