...
usage() {
  cat <<'USAGE'
Usage:
  ./run_CstarToGJ_rscan.sh [options]
...
Examples:
  ./run_CstarToGJ_rscan.sh -f f1p0
  ./run_CstarToGJ_rscan.sh -f f0p5 --mass-list "1000 1600"
  ./run_CstarToGJ_rscan.sh -f f0p1 --points 200 --r-min -1 --r-max 5
...
Manual command for one mass point / coupling, e.g. M1000 f1p0:
  ...
  plot1DScan.py higgsCombine_scan_M1000_f1p0.MultiDimFit.mH1000.root \
...
for ws in "${workspaces[@]}"; do
  ...
  ws_path="${work_dir}/${ws}"
  scan_root="higgsCombine_scan_${label}.MultiDimFit.mH${mass}.root"
  scan_plot="scan_${label}"
  main_label="M_{c*}=${mass} GeV, f=$(coupling_label "$coupling")"
