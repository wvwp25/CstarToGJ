...
usage() {
  cat <<'USAGE'
Usage:
  ./run_CstarToGJ_impacts_expected_ranged.sh [options]
...
Options:
  ...
  --r-min VALUE          Minimum r value. Default: 0
  --r-max VALUE          Maximum r value. Default: 5
  --dry-run              Print commands without executing them
  -h, --help             Show this help

Examples:
  ./run_CstarToGJ_impacts_expected_ranged.sh -f f1p0
  ./run_CstarToGJ_impacts_expected_ranged.sh -f f0p5 --parallel 12 --r-min 0 --r-max 50
  ./run_CstarToGJ_impacts_expected_ranged.sh -f f0p1 --mass-list "1000 1200 1400" --r-min -2 --r-max 10
  ./run_CstarToGJ_impacts_expected_ranged.sh -f all
  ./run_CstarToGJ_impacts_expected_ranged.sh -f 'f*'
...
Manual command for one mass point / coupling, e.g. M1000 f1p0:
  ...
  combineTool.py -M Impacts \
    -d /path/to/workspace_M1000_f1p0.root \
    -m 1000 \
    -o impacts_expected_r1_M1000_f1p0.json \
    -t -1 \
    --expectSignal 1 \
    --rMin 0 \
    --rMax 5
  ...
  plotImpacts.py \
    -i impacts_expected_r1_M1000_f1p0.json \
    -o impacts_expected_r1_M1000_f1p0
USAGE
}
...
for ws in "${workspaces[@]}"; do
  ...
  mass="${mass#M}"
  coup="${label#*_}"
  output_dir="higgsCombine_expected_r${expect_signal}_${coup}"
  ws_path="${work_dir}/${ws}"
  output_json="impacts_expected_r${expect_signal}_${label}.json"
  output_plot="impacts_expected_r${expect_signal}_${label}"
